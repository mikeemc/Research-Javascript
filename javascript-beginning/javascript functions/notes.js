...
// Find info here: https://jsdoc.app/index.html
...
function createPlanetHTML(planet) {
  return `<div class="card"><div><img src="img/${planet.name}.jpg" alt="${planet.name}"></div><h2>${planet.name}</h2><p>${planet.description}</p><h3>Planet Profile</h3><ul><li><strong>Diameter: </strong>${planet.diameter}</li><li><strong>Moons: </strong>${planet.moons}</li><li><strong>Temperature: </strong>${planet.temp}</li><li><strong>Orbit Period: </strong>${planet.orbitDays} days (${planet.orbitYears} years)</li></ul><p>${planet.facts}</p></div>`;
}

// document.querySelector('body').innerHTML = createPlanetHTML(mars); Change the word mars with another planets.name like saturn
